 ...
 *
 * Uses Web Crypto API for SHA algorithms (secure, built-in)
 * Includes a pure JS implementation for MD5 (legacy support)
 */
...
export type HashResponse = HashResult | HashError;
 ...
 * Included here for legacy compatibility and non-security use cases.
 */
 ...
 */
export async function generateHash(input: string, algorithm: HashAlgorithm): Promise<HashResponse> {
  try {
    if (!input) {
    ...
    if (algorithm === 'MD5') {
      ...
      hash = await generateShaHash(input, algorithm);
    }
...
/**
 * Generate all hashes for the given input string
 */
export async function generateAllHashes(
  input: string
): Promise<{ algorithm: HashAlgorithm; hash: string }[]> {
  if (!input) return [];
  ...
  const results = await Promise.all(
    algorithms.map(async (algorithm) => {
      ...
      return {
        algorithm,
        hash: result.success ? result.hash : '',
      };
    })
  );
...
/**
 * Get hash length in characters for a given algorithm
 */
