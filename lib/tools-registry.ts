...
export const tools: Tool[] = [
  // ── Data Inspection & Formatting ──────────────────────────────
  {
    ...
    category: 'Formatters',
    tier: 'core',
    workflow: 'Data Inspection & Formatting',
  },
  {
    slug: 'json-minifier',
    name: 'JSON Minifier',
...
export const getCoreTools = () => tools.filter((t) => t.tier === 'core');
export const getSupportingTools = () => tools.filter((t) => t.tier === 'supporting');
export const getBackgroundTools = () => tools.filter((t) => t.tier === 'background');
export const getVisibleTools = () => tools.filter((t) => t.tier !== 'background');

export const getToolsByWorkflow = (workflow: ToolWorkflow) =>
  tools.filter((t) => t.workflow === workflow);

export const getWorkflows = (): ToolWorkflow[] =>
  Array.from(new Set(tools.map((t) => t.workflow))) as ToolWorkflow[];
